...
class MarkovChainAnalytics:
    ...
    def __init__(self, markov_chain):
        ...
        # Use direct JSON logging if available
        if log_json:
            log_json(logger, "MarkovChainAnalytics initialized", {
                'model_type': 'markov_chain',
                'n_gram': self.markov_chain.n_gram,
                'storage_type': 'database' if self.markov_chain.using_db else 'memory',
                'environment': self.markov_chain.environment,
            ...
            logger.info("MarkovChainAnalytics initialized", extra=extra)
    ...
    def analyze_model(self):
        ...
        if self.markov_chain.using_db:
            ...
            # In-memory analytics
            stats["transitions_count"] = sum(len(next_words) for next_words in self.markov_chain.transitions.values())
            ...
            total_transitions = sum(len(next_words) for next_words in self.markov_chain.transitions.values())
            if stats["states_count"] > 0:
                stats["avg_transitions_per_state"] = total_transitions / stats["states_count"]
            else:
                stats["avg_transitions_per_state"] = 0
                
            # Get memory usage estimate
            import sys
            memory_size = sum(sys.getsizeof(state) + sys.getsizeof(next_words) 
                             for state, next_words in self.markov_chain.transitions.items())
        ...
        # Log results
        if log_json:
            log_json(logger, "Model analysis completed", {
                "model_metrics": stats,
            ...
            logger.info(f"Model analysis completed in {execution_time:.3f}s", extra=extra)
    ...
    def get_transition_probability(self, current_state, next_word):
        ...
        if self.markov_chain.using_db:
            prob = self._get_db_transition_probability(current_state, next_word)
        else:
            prob = self._get_memory_transition_probability(current_state, next_word)
            
        # Log metrics
        execution_time = time.time() - start_time
        
        if log_json:
            log_json(logger, f"Transition probability: {current_state} → {next_word}", {
            ...
            logger.debug(f"Transition probability: {current_state} → {next_word} = {prob}", extra=extra)
    ...
    def _get_db_transition_probability(self, current_state, next_word):
        ...
        try:
            ...
            logger.error(f"Error calculating transition probability: {e}", extra=extra)
            return 0.0
        finally:
            self.markov_chain._return_connection(conn)
    ...
    def score_sequence(self, sequence, preprocess=True):
        ...
        if log_json:
            log_json(logger, "Starting sequence scoring", {
            ...
            logger.debug(f"Starting sequence scoring", extra=extra)
        ...
        if len(words) <= self.markov_chain.n_gram:
            # Log early return
            if log_json:
                log_json(logger, "Sequence too short for scoring", {
                    'score': 0.0,
        ...
        for i in range(len(words) - self.markov_chain.n_gram):
            ...
            transition_prob = self.get_transition_probability(current_state, next_word)
        ...
        # Log results
        if log_json:
            log_json(logger, f"Sequence scored with result {final_score:.4f}", {
                'sequence': sequence,
                'score': final_score,
            ...
            logger.info(f"Sequence scored with result {final_score:.4f}", extra=extra)
    ...
    def perplexity(self, text, preprocess=True):
        ...
        if log_json:
            log_json(logger, "Starting perplexity calculation", {
            ...
            logger.info(f"Starting perplexity calculation", extra=extra)
        ...
        if len(words) <= self.markov_chain.n_gram:
            ...
            if log_json:
                log_json(logger, "Text too short for perplexity calculation", {
        ...
        # Log results
        if log_json:
            log_json(logger, f"Perplexity calculation complete: {perplexity:.4f}", {
                'text_id': text_id,
            ...
            logger.info(f"Perplexity calculation complete: {perplexity:.4f}", extra=extra)
    ...
    def find_high_probability_sequences(self, length=3, top_n=10):
        ...
        if log_json:
            log_json(logger, f"Finding high probability sequences of length {length}", {
            ...
            logger.info(f"Finding high probability sequences of length {length}", extra=extra)
        
        if self.markov_chain.using_db:
            results = self._find_high_probability_sequences_db(length, top_n)
        else:
            results = self._find_high_probability_sequences_memory(length, top_n)
        ...
        result_metrics = []
        for i, (sequence, probability) in enumerate(results[:5]):  # Log only top 5 for brevity
        ...
        # Log results
        if log_json:
            log_json(logger, f"Found {len(results)} high probability sequences", {
                'execution_time': execution_time,
            ...
            logger.info(f"Found {len(results)} high probability sequences in {execution_time:.3f}s", extra=extra)
    ...
    def _find_high_probability_sequences_memory(self, length=3, top_n=10):
            ...
            if self.markov_chain.n_gram == 1:
                ...
                for _ in range(top_n * 2):  # Sample more than needed and take top_n
                    ...
                    for _ in range(length - 1):
                        ...
                        next_word_probs = [count / total for _, count in next_word_items]
                ...
                # Try to build sequences of desired length
                for _ in range(min(top_n * 2, 50)):  # Sample some sequences from each start state
                    ...
                    for _ in range(length - len(sequence)):
                        ...
                        if len(sequence) >= self.markov_chain.n_gram:
                            current_state = tuple(sequence[-(self.markov_chain.n_gram):])
        ...
        top_sequences = sorted(sequences_found, key=lambda x: x[1], reverse=True)[:top_n]
        ...
        try:
            ...
            with conn.cursor() as cur:
                ...
                for db_state in start_states:
                    ...
                    for _ in range(min(top_n, 10)):  # Sample some sequences from each start state
                        if self.markov_chain.n_gram == 1:
                            sequence = [current_state]
                        else:
                        ...
                        for _ in range(length - len(sequence)):
                            ...
                            next_word_probs = [count / total for _, count in next_words]
                            next_word_idx = self._weighted_choice(next_word_probs)
