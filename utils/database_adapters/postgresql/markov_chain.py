...
class MarkovChainPostgreSqlAdapter:
    ...
    def _initialize_connection_pool(self):
        ...
        try:
            if not self.db_config:
                if self.logger:
                    self.logger.warning(
                        "Cannot initialize connection pool - no config provided")
                return False
            ...
            for param in required_params:
                if param not in self.db_config:
                    if self.logger:
                        self.logger.warning(
                            f"Missing required database parameter: {param}")
                    return False
    ...
    def get_connection(self):
        ...
        if self.conn_pool:
            try:
                ...
                if self.logger:
                    self.logger.error(
                        f"Error getting connection from pool: {e}")
                return None
        return None
    ...
    def insert_transitions_batch(self, batch, n_gram):
        ...
        if not conn:
            if self.logger:
                self.logger.warning(
                    "Failed to get connection for batch insert")
            return False
    ...
    def get_transition_probability(self, current_state, next_word, n_gram):
        ...
        try:
            ...
            if self.logger:
                self.logger.error(
                    f"Error calculating transition probability: {e}")
            return 0.0
    ...
    def find_high_probability_sequences(self, n_gram, length=3, top_n=10):
        ...
        try:
            ...
            if self.logger:
                self.logger.error(
                    f"Error finding high probability sequences: {e}")
            return []
