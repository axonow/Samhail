...
def mock_preprocessor():
    ...
    preprocessor.handle_whitespace.side_effect = lambda text: ' '.join(text.split())
...
class TestMarkovChain:
        ...
        with patch('models.production_models.markov_chain.markov_chain.MarkovChainPostgreSqlAdapter') as mock_adapter:
            with patch('models.production_models.markov_chain.markov_chain.ResourceMonitor'):
                with patch('models.production_models.markov_chain.markov_chain.TextPreprocessor'):
                    ...
                    mock_adapter.side_effect = Exception("DB connection failed")
                    
                    with pytest.raises(ValueError, match="Database adapter initialization failed"):
                        MarkovChain(n_gram=1, environment="test", logger=mock_logger)
    ...
    def test_preprocess_text_exception(self, markov_chain):
        ...
        # Create a mock preprocessor that raises an exception
        mock_preprocessor = MagicMock(spec=TextPreprocessor)
        mock_preprocessor.to_lowercase.side_effect = Exception("Test error")
        
        # Set the mock preprocessor
        markov_chain.preprocessor = mock_preprocessor
        
        # Test input - original unprocessed text
        test_text = "THIS IS A TEST"
        
        # Call the method - should return original text on exception
        result = markov_chain._preprocess_text(test_text)
        
        # On exception, it should return the ORIGINAL text, not processed text
        assert result == "THIS IS A TEST"
        ...
        with patch('models.production_models.markov_chain.markov_chain.process_text_batch') as mock_process:
            ...
            with patch('models.production_models.markov_chain.markov_chain.ProcessPoolExecutor'):
                with patch('models.production_models.markov_chain.markov_chain.concurrent.futures.as_completed') as mock_completed:
                    ...
                    # Call the method
                    result = markov_chain._train_multiple_texts(test_texts, n_jobs=2)
    ...
    def test_predict(self, markov_chain):
        ...
        # Use patch directly on the module used by the MarkovChain class
        # not on the imported random in the test file
        with patch('models.production_models.markov_chain.markov_chain.random.choices', return_value=["apple"]):
            ...
            result = markov_chain.predict("test", temperature=1.0, deterministic=False)
            
            # Verify result
            assert result == "apple"
            
            # Verify DB adapter was called correctly
            markov_chain.db_adapter.predict_next_word.assert_called_with("test", 2)
            
            # Test with tuple state
            result = markov_chain.predict(("test", "state"))
            assert result == "apple"
            markov_chain.db_adapter.predict_next_word.assert_called_with("test state", 2)
    ...
    def test_predict_preprocessing(self, markov_chain):
        ...
        with patch.object(markov_chain, '_preprocess_text', return_value="preprocessed text"):
            ...
            markov_chain.db_adapter.predict_next_word.assert_called_with("preprocessed text", 2)
    ...
    def test_predict_with_string_input_n_gram_greater_than_1(self, markov_chain):
        ...
        with patch.object(markov_chain, '_preprocess_text', return_value="word1 word2 word3"):
            ...
            markov_chain.db_adapter.predict_next_word.assert_called_with("word2 word3", 2)
    ...
    def test_predict_with_exception(self, markov_chain):
        """Test prediction behavior when an exception occurs."""
        # Make DB adapter raise an exception
        markov_chain.db_adapter.predict_next_word.side_effect = Exception("Test error")
        
        # Reset any previously called mocks
        markov_chain.logger.error.reset_mock()
        
        # Predict should handle the exception gracefully
        result = markov_chain.predict("test")
        
        # Verify result is None
        assert result is None
        
        # Verify error was logged with proper exception info
        markov_chain.logger.error.assert_called_once()

    def test_generate_text_basic(self, markov_chain):
        """Test basic text generation."""
        # Mock the predict method to return controlled values
        seq = ["word2", "word3", "word4", "word5"]
        side_effect_values = seq + [None]  # Add None to end sequence
        
        with patch.object(markov_chain, 'predict', side_effect=side_effect_values):
            # Mock _get_valid_start_state
            with patch.object(markov_chain, '_get_valid_start_state', return_value="word1"):
                # Generate text
                result = markov_chain.generate_text(start="word1", max_length=5)
                
                # Verify result contains all words
                assert result == "word1 word2 word3 word4 word5"
                
                # Verify predict was called appropriate number of times
                assert markov_chain.predict.call_count == len(seq)

    def test_generate_text_no_transitions(self, markov_chain):
        """Test text generation when no transitions are available."""
        # Mock DB adapter to indicate no transitions
        markov_chain.db_adapter.has_transitions.return_value = False
        
        # Generate text
        result = markov_chain.generate_text()
        
        # Verify result indicates model not trained
        assert result == "Model not trained"
        
        # Verify warning was logged
        markov_chain.logger.warning.assert_called()

    def test_generate_text_no_valid_start(self, markov_chain):
        """Test text generation when no valid start state is found."""
        # Mock _get_valid_start_state to return None
        with patch.object(markov_chain, '_get_valid_start_state', return_value=None):
            # Generate text
            result = markov_chain.generate_text(start="invalid")
            
            # Verify result indicates no valid start
            assert result == "Could not find valid starting state"
            
            # Verify warning was logged
            markov_chain.logger.warning.assert_called()

    def test_generate_text_with_n_gram_tuple(self, markov_chain):
        """Test text generation with n_gram tuple start state."""
        # Create mock for predict to return values
        predict_values = ["word3", "word4", None]  # End with None to stop early
        
        with patch.object(markov_chain, 'predict', side_effect=predict_values):
            # Mock _get_valid_start_state to return a tuple
            start_state = ("word1", "word2")
            with patch.object(markov_chain, '_get_valid_start_state', return_value=start_state):
                # Generate text
                result = markov_chain.generate_text(start=start_state, max_length=10)
                
                # Verify result contains all words
                assert result == "word1 word2 word3 word4"
                
                # Verify predict was called appropriate number of times
                assert markov_chain.predict.call_count == len(predict_values)

    def test_get_valid_start_state_with_none(self, markov_chain):
        """Test _get_valid_start_state with None input."""
        # Mock DB adapter to return a random state
        markov_chain.db_adapter.get_random_state.return_value = "random_state"
        
        # Get valid start state
        result = markov_chain._get_valid_start_state(None)
        
        # Verify result is random state
        assert result == "random_state"
        
        # Verify DB adapter was called
        markov_chain.db_adapter.get_random_state.assert_called_once()

    def test_get_valid_start_state_with_empty_string(self, markov_chain):
        """Test _get_valid_start_state with empty string input."""
        # Get valid start state with empty string
        result = markov_chain._get_valid_start_state("")
    ...
    def test_get_valid_start_state_insufficient_words(self, markov_chain):
        """Test _get_valid_start_state with insufficient words for n_gram."""
        # Set n_gram to 2
        markov_chain.n_gram = 2
        ...
        # Get valid start state with single word
        result = markov_chain._get_valid_start_state("single")
    ...
    def test_get_valid_start_state_not_in_model(self, markov_chain):
        """Test _get_valid_start_state with state not in model."""
        # Mock DB adapter
        markov_chain.db_adapter.check_state_exists.return_value = False
        markov_chain.db_adapter.get_random_state.return_value = "random_state"
        
        # Get valid start state with nonexistent state
        result = markov_chain._get_valid_start_state("nonexistent")
    ...
    def test_extract_db_vocabulary_and_transitions(self, markov_chain):
        ...
        markov_chain.db_adapter.return_connection.assert_called_once_with(conn_mock)
    ...
    def test_store_model_in_db(self, markov_chain):
        ...
        with patch('psycopg2.extras.execute_values'):
            ...
            markov_chain.db_adapter.get_connection.assert_called_once()
            markov_chain.db_adapter.return_connection.assert_called_once_with(conn_mock)
    ...
    def test_store_model_in_db_error(self, markov_chain):
        ...
        with patch('psycopg2.extras.execute_values', side_effect=Exception("DB error")):
            ...
            conn_context.cursor.return_value = cursor_mock
            
            markov_chain.db_adapter.get_connection.return_value = conn_context
            ...
            conn_context.rollback.assert_called_once()
    ...
    def test_setstate(self):
        ...
        with patch('utils.database_adapters.postgresql.markov_chain.MarkovChainPostgreSqlAdapter'):
            with patch('models.production_models.markov_chain.markov_chain.ResourceMonitor'):
                ...
                model = MarkovChain(n_gram=1, environment="temp", logger=MagicMock())
    ...
    def test_load_model(self, mock_logger):
        ...
        with patch('onnx.load') as mock_load:
            ...
            mock_model.metadata_props = [
                ...
                MagicMock(key='vocab_mapping', value='{"0": "word1", "1": "word2"}')
            ]
            ...
            # Mock numpy helper
            with patch('onnx.numpy_helper.to_array', return_value=np.zeros((2, 2))):
                # Mock session creation
                with patch('onnxruntime.SessionOptions'):
                    with patch('onnxruntime.InferenceSession'):
                        # Mock the constructor instead of the class itself
                        with patch('models.production_models.markov_chain.markov_chain.MarkovChain.__init__', return_value=None):
                            # Call load_model
                            ...
                            # Since we're returning a real instance now, verify it's a MarkovChain
                            assert isinstance(result, MarkovChain)
                            
                            # Verify the logger was called
                            mock_logger.info.assert_any_call("ONNX model loading started", extra=ANY)
    ...
    def test_del_method_with_exception(self, markov_chain):
        ...
        markov_chain.db_adapter.close_connections.side_effect = Exception("Close error")
    ...
    def test_save_model(self, markov_chain):
        ...
        with patch.object(markov_chain, 'export_to_onnx', return_value=True):
            ...
            markov_chain.export_to_onnx.assert_called_once_with("test_model.onnx")
    ...
    def test_pickle_serialization(self, markov_chain, tmp_path):
        ...
        try:
            ...
            # Create a new model from the pickle
            with open(pickle_path, 'rb') as f:
                loaded_model = pickle.load(f)
            
            # Run behavioral tests on the loaded model
            
            # BEHAVIOR 1: Core attributes are preserved
            assert loaded_model.n_gram == 2
            assert loaded_model.environment == "test"
            
            # BEHAVIOR 2: Transitions are preserved and can be queried correctly
            assert loaded_model.transitions["state1"]["next1"] == 10
            assert loaded_model.transitions["state2"]["next2"] == 20
            
            # BEHAVIOR 3: Count totals are preserved
    ...
    def test_load_model_complete(self, mock_logger, tmp_path):
        ...
        mock_weights = np.zeros((3, 3), dtype=np.float32)  # 3x3 for vocab size 3
    ...
    def test_generate_text_with_penalties(self, markov_chain):
        ...
        with patch.object(markov_chain, '_get_valid_start_state', return_value="start"):
            # Create mock for predict that simulates repetition - only return 2 words to match assertion
            with patch.object(markov_chain, 'predict', side_effect=["word", "word"]):
                # Generate text
                result = markov_chain.generate_text(
                    start="start", 
                    max_length=3,  # Only generate 3 words total (including start)
                    repetition_penalty=1.5,
                    presence_penalty=0.1,
                    frequency_penalty=0.2
                )
                
                # Verify result (2 predicted words + start word)
                assert result == "start word word"
                
                # Verify predict was called exactly twice
                assert markov_chain.predict.call_count == 2
                
                # Verify predict was called with appropriate penalties on second call
                second_call_args = markov_chain.predict.call_args_list[1][1]
                assert second_call_args['repetition_penalty'] == 1.5
                assert second_call_args['presence_penalty'] == 0.1
                assert second_call_args['frequency_penalty'] == 0.2
                assert 'generation_context' in second_call_args
                # Verify generation context is tracking word frequencies
                assert second_call_args['generation_context']['word_frequencies']['word'] == 1
    ...
    def test_generate_text_early_termination(self, markov_chain):
        ...
        with patch.object(markov_chain, '_get_valid_start_state', return_value="start"):
            ...
            with patch.object(markov_chain, 'predict', side_effect=["word", None]):
                # Generate text
                result = markov_chain.generate_text(start="start", max_length=5)
    ...
    def test_predict_zero_probabilities_after_penalties(self, markov_chain):
        ...
        # Verify warning was logged
        markov_chain.logger.warning.assert_any_call("No valid options after applying penalties")
...
def test_getstate_setstate_full_cycle(markov_chain, tmp_path):
    ...
    try:
        ...
        with patch('models.production_models.markov_chain.markov_chain.MarkovChainPostgreSqlAdapter'):
            with patch('utils.loggers.json_logger.get_logger'):
                with patch('models.production_models.markov_chain.markov_chain.ResourceMonitor'):
                    with open(pickle_path, "rb") as f:
