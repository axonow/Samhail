...
def test_handle_emojis(preprocessor):
    text = "I love Python! 😊"
    assert preprocessor.handle_emojis(text) == "I love Python! :smiling_face_with_smiling_eyes:"
...
def test_sentence_segmentation(preprocessor):
    ...
    assert preprocessor.sentence_segmentation(text) == ["This is the first sentence.", "Here is another one."]
...
def test_detect_language(preprocessor):
    ...
    # Empty text
    text = ""
    with pytest.raises(Exception):  # Language detection might raise an exception for empty input
